<x-app-layout>
    @section('sidebar#artikel', 'border-b-2 border-gray-200')
        ...
        <div class="relative w-full p-4 overflow-x-auto bg-white rounded-lg shadow-md">
            <div class="grid items-center justify-between px-8 mb-8 lg:flex">
                <div class="max-lg:mb-4">
                    <h1 class="mb-1 text-4xl font-bold">Artikel</h1>
                    <span class="font-medium">Daftar artikel yang Anda buat</span>
                </div>
                    ...
                    <tbody>
                        @foreach ($artikel as $item)
                            <tr class="bg-white border-b hover:bg-gray-50 ">
                                ...
                                <td class="gap-4 px-4 text-center">
    ...
    <div id="static-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-3xl max-h-full p-4">
            ...
            <div class="relative bg-white rounded-lg shadow ">
                ...
                <!-- Modal body -->
                <form action="{{ route('Artikel.store') }}" method="POST" class="w-full p-4 space-y-4 md:p-5">
                    @csrf
                    ...
                    <div class="mb-5">
                        <label for="id_penyakit" class="block mb-2 text-sm font-medium text-gray-900">Kategori
                            Penyakit</label>
                        <select id="id_penyakit" name="id_penyakit"
                            data-hs-select='{
        ...
        <div id="modal-edit-{{ $item->id }}" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-xl max-h-full p-4">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('Artikel.update', $item->id) }}" method="POST" class="p-4 md:p-5">
                        @csrf
                        @method('PUT')
                        <div class="mb-5">
                            <label for="judul" class="block mb-2 text-sm font-medium text-gray-900">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="isi" rows="4" name="isi"
                                class="block p-2.5 w-full text-sm text-gray-900 min-h-80 bg-gray-50 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500"
                                placeholder="Tulis isi penyakit . . ." required>{{ $item->isi }}</textarea>
                            @error('isi')
                                <p id="filled_error_help" class="mt-1 text-xs text-red-600 ">
                                    Tulis isi artikel penyakit !
                                </p>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-2 mb-5">
                            ...
                            "placeholder": "Pilih kaetori penyakit . . .",
                                ...
                                class="hidden">
                                <option value="">Choose</option>
                                @foreach ($penyakit as $kategori)
                                    <option value="{{ $kategori->id }}"
                                        {{ $kategori->id == $item->id_penyakit ? 'selected' : '' }}>
                                        {{ $kategori->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <form action="{{ route('Artikel.destroy', $item->id) }}" method="POST"
                        ...
                        <h3 class="mb-5 text-lg font-normal text-gray-500 ">
                            Apakah Anda Yakin ingin menghapus data penyakit <span
                                class="font-bold text-black">[{{ $item->judul }}]</span> ini?
                        </h3>
                        <button type="submit"
